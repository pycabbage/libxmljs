...
RUN source $NVM_DIR/nvm.sh && \
  corepack yarn global add @mapbox/node-pre-gyp node-gyp

COPY --chown=$USER . $HOME/libxmljs

WORKDIR $HOME/libxmljs
RUN rm -fr ./node_modules/ .yarn/cache/ .yarn/unplugged/
RUN source $NVM_DIR/nvm.sh && corepack yarn install
RUN source $NVM_DIR/nvm.sh && corepack yarn build
RUN source $NVM_DIR/nvm.sh && corepack yarn package
